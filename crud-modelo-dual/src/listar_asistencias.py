...
class DecimalEncoder(json.JSONEncoder):
    def default(self, o):
        if isinstance(o, Decimal):
            # Si tiene decimales lo hace float, si no, lo hace int
...
def lambda_handler(event, context):
    try:
        ...
        path = event.get('path', '') # Usamos el path completo para determinar la acción
        ...
        if '/incidencias/fecha/' in path:
            ...
            incidencias = respuesta.get('Items', [])
            resultados = [item for item in incidencias if item.get('estadoIncidencia') != 'ANULADA']
            ...
            if query_params.get('buscar'):
                termino = query_params['buscar'].lower()
                resultados = [
                    r for r in resultados 
                    if termino in r.get('alumno', {}).get('nombreCompleto', '').lower() or
                        termino in r.get('alumno', {}).get('numeroControl', '').lower()
                ]
            
            if query_params.get('carreraId'):
                c_id = str(query_params['carreraId'])
                resultados = [r for r in resultados if r.get('alumno', {}).get('carrera', {}).get('id') == c_id]
